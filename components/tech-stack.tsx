            ...
            <div className="mx-auto max-w-5xl px-6">
                <div className="aspect-16/10 group relative mx-auto flex max-w-[22rem] items-center justify-between sm:max-w-sm">
                    ...
                    <div className="bg-linear-to-b from-muted-foreground/15 absolute inset-0 flex aspect-square items-center justify-center rounded-full border-t to-transparent to-25%">
                        {/* Top */}
                        <TechCard className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2">
                            <Image src="/tech/tanstack.png" alt="TanStack" width={40} height={40} className="size-full object-contain" />
                        </TechCard>

                        {/* Top Right */}
                        <TechCard className="absolute right-[15%] top-[15%] translate-x-1/2 -translate-y-1/2">
                            <Image src="/tech/nextjs.png" alt="Next.js" width={40} height={40} className="size-full object-contain" />
                        </TechCard>

                        {/* Right Bottom */}
                        <TechCard className="absolute right-0 top-1/2 translate-x-1/2 -translate-y-1/2">
                            <Image src="/tech/typescript.png" alt="TypeScript" width={40} height={40} className="size-full object-contain" />
                        </TechCard>

                        {/* Left Top */}
                        <TechCard className="absolute left-[15%] top-[15%] -translate-x-1/2 -translate-y-1/2">
                            <Image src="/tech/react.png" alt="React" width={40} height={40} className="size-full object-contain" />
                        </TechCard>

                        {/* Left Bottom */}
                        <TechCard className="absolute left-0 top-1/2 -translate-x-1/2 -translate-y-1/2">
                    ...
                    <div className="bg-linear-to-b from-muted-foreground/15 absolute inset-16 flex aspect-square scale-90 items-center justify-center rounded-full border-t to-transparent to-25%">
                        ...
                        {/* Left Bottom */}
                        <TechCard className="absolute bottom-1/2 left-0 translate-y-1/2 -translate-x-1/2">
...
const TechCard = ({ children, className, isCenter = false }: { children: React.ReactNode; className?: string; isCenter?: boolean }) => {
    return (
        <div className={cn('relative z-30 flex size-12 rounded-full border bg-white shadow-sm shadow-black/5 dark:bg-white/5 dark:backdrop-blur-md items-center justify-center p-2', className)}>
